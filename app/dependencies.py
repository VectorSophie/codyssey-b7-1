...
from app.errors import ADMIN_REQUIRED, AUTH_REQUIRED, AppError
from app.models.user import User
from app.services.auth import verify_session_token

__all__ = ["get_db", "get_current_user", "require_auth", "require_admin"]
...
def require_admin(user: User = Depends(require_auth)) -> User:
    if user.username not in settings.admin_usernames:
        raise AppError(ADMIN_REQUIRED, "admin access required", status_code=403)
    return user
